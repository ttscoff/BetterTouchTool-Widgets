...
#║                                                                            ║
#╚============================================================================╝
# Reads all of the Bunches in your current Bunch Folder and creates
# BetterTouchTool Touch Bar widgets for them. Each widget shows the name of the
# Bunch and the open/closed state using background colors. Requires additional
# setup: <http://ckyp.us/jcBxaM>
#
# A group will be created with the Bunch icon as a label. You can add icons for
# each Bunch as desired. By default, this script does not add polling for the
# status updates, and will only set the state when the widget becomes visible.
# The intention is that, using the resource linked above, you'll have Bunch
# refresh the state of its widgets when opening or closing them, avoiding the
# need for background polling. If you change the "Execute script every..."
# field in the button's Widget Specific configuration to a positive integer,
# it will poll for status instead.
#
...
#║                                                                            ║
#╚============================================================================╝
# 1. If needed, copy `bunch_status.rb` to a permanent location.
# 2. Make the status script executable with `chmod a+x /path/to/bunch_status.rb`
...
#║                    |___/                                                   ║
#╚============================================================================╝
# 1. Edit the location of `status_script` below to point to `bunch_status.rb`
status_script = '/Users/ttscoff/scripts/bunch_status.rb'
# 2. If using Bunch Beta, change the below to "Bunch Beta"
bunch_app = 'Bunch'
#
#### END CONFIG ###
...
status_script = File.expand_path(status_script)
raise "Status script not found, please edit the `status_script` line in #{__FILE__}" unless File.exist?(status_script)
raise %(Status script not executable, please run `chmod a+x "#{status_script}"`) unless File.executable?(status_script)

group = JSON.parse(DATA.read)
